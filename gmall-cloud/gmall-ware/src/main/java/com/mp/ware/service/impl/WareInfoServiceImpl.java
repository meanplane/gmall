...
import com.mp.ware.mapper.WareInfoMapper;
import com.mp.ware.service.WareInfoService;
...
public class WareInfoServiceImpl extends ServiceImpl<WareInfoMapper, WareInfo> implements WareInfoService {
    ...
    public PageUtils queryPage(Map<String, Object> params) {
        ...
        QueryWrapper<WareInfo> wareInfoQueryWrapper = new QueryWrapper<>();
        String key = (String) params.get("key");
        if(!StringUtils.isEmpty(key)){
            wareInfoQueryWrapper.eq("id",key).or()
                    .like("name",key)
                    .or().like("address",key)
